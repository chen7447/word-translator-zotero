"""Qwen-MT via DashScope OpenAI-compatible mode probe. Proxy: 127.0.0.1:7897.
"""
...
def build_request(bad_path=False, missing_msg=False):
    endpoint = ENDPOINT + ("-wrong" if bad_path else "")
    body = {"model": "qwen-mt-plus",
            "messages": [{"role": "user", "content": "" if missing_msg else "Translate environment into Chinese."}]}
    return endpoint, body

def send(endpoint, body):
    cmd = ["curl.exe", "-k", "-sS", "-X", "POST", endpoint,
           "-H", "Authorization: Bearer TEST_DASHSCOPE_KEY",
...
def main():
    p = argparse.ArgumentParser(); p.add_argument("--send", action="store_true"); p.add_argument("--bad-path", action="store_true"); p.add_argument("--missing-msg", action="store_true")
    a = p.parse_args(); endpoint, body = build_request(a.bad_path, a.missing_msg)
    print(json.dumps({"dry_run": not a.send, "endpoint": endpoint, "body_keys": sorted(body)}, indent=2))
    if not a.send: return
    result = send(endpoint, body); path = write_result("qwenmt", {"probe": vars(a), **result})
