"""LibreTranslate protocol probe. Default localhost:5000. Proxy: 127.0.0.1:7897.
"""
...
def build_request(bad_path=False, missing_q=False):
    ...
    body = {"q": "" if missing_q else "environment", "source": "en", "target": "zh", "format": "text"}
    return endpoint, body

def send(endpoint, body):
    cmd = ["curl.exe", "-k", "-sS", "-X", "POST", endpoint, "-H", "Content-Type: application/json", "--data", json.dumps(body), "-w", "\n__HTTP_STATUS__:%{http_code}", "--max-time", "30"]
    try:
...
def main():
    ...
    a = p.parse_args(); endpoint, body = build_request(a.bad_path, a.missing_q)
    print(json.dumps({"dry_run": not a.send, "endpoint": endpoint, "body_keys": sorted(body)}, indent=2))
    if not a.send: return
    result = send(endpoint, body); path = write_result("libretranslate", {"probe": vars(a), **result})
