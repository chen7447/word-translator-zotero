"""Microsoft Azure Translator protocol probe. Proxy: 127.0.0.1:7897. No plugin code.
"""
...
ENDPOINT = "https://api.cognitive.microsofttranslator.com/translate?api-version=3.0&to=zh"
...
def send(endpoint, body):
    cmd = ["curl.exe", "-k", "-sS", "-X", "POST", endpoint, "-H", "Ocp-Apim-Subscription-Key: TEST_MS_KEY", "-H", "Content-Type: application/json", "--data", json.dumps(body), "-w", "\n__HTTP_STATUS__:%{http_code}", "--max-time", "30"]
    try:
...
def main():
    ...
    result = send(endpoint, body); path = write_result("microsoft", {"probe": vars(a), **result})
